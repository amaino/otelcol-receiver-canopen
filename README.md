...
1. **First commit** — a bare OpenTelemetry Collector distribution (built via
...
2. **This commit** — the `canopen` receiver, supporting only passive traffic
...
- [`receiver/canopenreceiver`](./receiver/canopenreceiver) — the
  contrib-layout receiver module, currently limited to passive CANopen
  traffic sniffing.
...
- [`.github/workflows/ci.yaml`](./.github/workflows/ci.yaml) — CI that
  builds, tests, and cross-compiles the passive receiver, then builds the
  `otelcol-canopen` distribution via ocb.
- [`cmd/canopen-publisher`](./cmd/canopen-publisher) — a test publisher
  that simulates heartbeat, TPDO, and optional EMCY traffic on `vcan0`.
